...
public partial class Name : ValueObject<string>
{
    ...
    public static Name Create(string value)
    {
        ...
        if (value.Length < MinLength || value.Length > MaxLength)
            throw new InvalidValueObjectException(nameof(Name), $"must be between {MaxLength} and {MinLength} characters.");

        if (!NameRegex().IsMatch(value))
            throw new InvalidValueObjectException(nameof(Name), "can only contain letters, numbers, and underscores.");
