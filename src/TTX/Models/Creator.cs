...
public class Creator : User
{
    ...
    public StreamStatus StreamStatus { get; init; } = new StreamStatus();
    public HashSet<Transaction> Transactions { get; init; } = [];
    public HashSet<Vote> History { get; set; } = [];

    public ImmutableArray<Share> GetShares()
    {
        var shares = new Dictionary<ModelId, Share>();
        foreach (var tx in Transactions)
        {
            var share = shares.GetValueOrDefault(tx.Player.Id, new Share
            {
                Creator = this,
                Player = tx.Player,
            });
    ...
    public void RecordTransaction(Transaction tx) => Transactions.Add(tx);
    ...
    public static Creator Create(Name name, Slug slug, TwitchId twitchId, Uri avatarUrl, Ticker ticker, Credits? value = null)
    {
