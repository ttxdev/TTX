...
export const handler = define.handlers({
  async GET(ctx) {
    ...
    // These four reads are independent, so fetch them concurrently instead of
    // in series — the page is only as slow as the single slowest call.
    const [featuredCreator, featuredCreators, topCreators, topPlayers] =
      await Promise.all([
        client.getCreator("atrioc"),
        client.getCreators(
          1,
          3,
          undefined,
