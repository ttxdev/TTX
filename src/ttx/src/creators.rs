...
//!
//! The service is the headline of the module; the domain model, persistence,
//! DTOs, and events live in the submodules below.
...
/// Translates `validator` errors into the crate's [`ValidationFailure`] list.
...
impl CreatorService {
    ...
    pub async fn index(&self, request: IndexCreatorsRequest) -> Result<PaginationDto<CreatorDto>> {
        ...
        // Index does not load transactions, so shares/transactions are empty.
    ...
    pub async fn onboard(&self, request: OnboardRequest) -> Result<Id> {
        // NOTE: the C# `OnboardValidator` and service both inverted their
        // checks (`if (result.IsValid) return Err(...)` and a uniqueness rule
        // that passed when the ticker already existed). This port implements
        // the evident intent: reject invalid requests and non-unique tickers.
