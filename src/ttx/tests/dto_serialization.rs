...
fn creator_partial_dto_json_shape() {
    ...
    // platform_id is serialized as a string (domain stores it as a number).
    assert_eq!(v["platform_id"], json!("5"));
    // platform_url is computed from the slug.
...
fn player_partial_dto_renames_kind_to_type() {
    ...
    assert_eq!(v["value"], json!(100.0)); // credits + portfolio(0)
}
